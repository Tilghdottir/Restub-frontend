...
function EventCard({ event, onClick }: { event: typeof EVENTS[0]; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
    ...
    >
      <div className="relative bg-[#E8E9EB] overflow-hidden" style={{ aspectRatio: '3/2' }}>
        <img
          src={event.image}
          alt={event.title}
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3 mb-1">
          <div className="min-w-0">
            <p className="text-[11px] font-medium text-[#292B2F] tracking-wide uppercase mb-1">{event.category}</p>
            <h3 className="text-[15px] font-semibold text-[#111111] leading-tight truncate">{event.title}</h3>
            <p className="text-[13px] text-[#292B2F] mt-0.5 truncate">{event.subtitle}</p>
          </div>
        </div>
        <div className="mt-3 pt-3 border-t border-[#E8E9EB] flex items-end justify-between">
          <div>
            <p className="text-[11px] text-[#292B2F]">{event.date} · {event.time}</p>
            <p className="text-[11px] text-[#292B2F] mt-0.5">{event.venue}</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] text-[#292B2F] mb-0.5">from</p>
            <PriceTag value={event.minPrice} className="text-[15px] font-semibold text-[#111111]" />
          </div>
        </div>
      </div>
    </button>
  ...
  return (
    ...
    >
      <div className="min-w-0 flex-1">
        <p className={`text-[13px] font-semibold ${selected ? 'text-white' : 'text-[#111111]'}`}>{section.label}</p>
        <p className={`text-[11px] mt-0.5 font-mono-data ${selected ? 'text-[#FAFAF7]/70' : 'text-[#292B2F]'}`}>
          Row {section.row} · Seats {section.seats}
        </p>
      </div>
      <div className="text-right shrink-0">
        <PriceTag value={section.price} className={`text-[15px] font-semibold ${selected ? 'text-white' : 'text-[#111111]'}`} />
        <p className={`text-[10px] mt-0.5 ${selected ? 'text-[#FAFAF7]/60' : 'text-[#292B2F]'}`}>ea · {section.qty} avail</p>
      </div>
    </button>
  )
}
    ...
    return (
      <div className="min-h-screen bg-[#FAFAF7] flex flex-col">
        ...
        <header className="bg-white border-b border-[#E8E9EB] px-6 py-3.5 flex items-center gap-4">
          ...
          <span className="text-[13px] text-[#292B2F] truncate">{selectedEvent.title} · {selectedEvent.date}</span>
        </header>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="relative bg-[#E8E9EB]" style={{ height: '280px' }}>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6">
                <p className="text-[11px] font-medium tracking-widest uppercase text-white/70 mb-1">{selectedEvent.category}</p>
                <h1 className="text-2xl font-bold text-white leading-tight">{selectedEvent.title}</h1>
                <p className="text-[14px] text-white/80 mt-1">{selectedEvent.subtitle}</p>
              </div>
            </div>
            ...
            <div className="px-6 py-4 bg-white border-b border-[#E8E9EB]">
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  <div key={label}>
                    <p className="text-[10px] font-medium tracking-wide uppercase text-[#292B2F] mb-1">{label}</p>
            ...
            <div className="px-6 pt-5 pb-3">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-[13px] font-semibold uppercase tracking-wide text-[#111111]">Select Tickets</h2>
                <div className="flex items-center gap-2 border border-[#E8E9EB] rounded-lg overflow-hidden">
                  ...
                  <span className="font-mono-data text-[13px] font-medium px-2 min-w-[2ch] text-center">{qty}</span>
                  <button
          ...
          <aside className="w-[280px] bg-[#111111] text-[#FAFAF7] flex flex-col p-6 hidden md:flex">
            <div className="flex-1">
              <h2 className="text-[11px] font-medium tracking-widest uppercase text-[#FAFAF7]/50 mb-5">Order Summary</h2>
              <div className="space-y-4">
                <div>
                  <p className="text-[11px] text-[#FAFAF7]/50 mb-1">Section</p>
                  <p className="text-[14px] font-semibold">{section?.label}</p>
                  <p className="text-[12px] text-[#FAFAF7]/60 font-mono-data mt-0.5">Row {section?.row} · Seats {section?.seats}</p>
                </div>
                <div>
                  <p className="text-[11px] text-[#FAFAF7]/50 mb-1">Quantity</p>
                  <p className="font-mono-data text-[14px] font-semibold">{qty} tickets</p>
                </div>
                <div className="border-t border-[#FAFAF7]/10 pt-4 space-y-2">
                  <div className="flex justify-between text-[12px]">
                    <span className="text-[#FAFAF7]/60">Tickets ({qty}×)</span>
                    <PriceTag value={section?.price * qty} className="text-[#FAFAF7]/80" />
                  </div>
                  <div className="flex justify-between text-[12px]">
        ...
        <div className="md:hidden bg-[#111111] text-[#FAFAF7] px-5 py-4">
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="text-[11px] text-[#FAFAF7]/50">{section?.label} · {qty} tickets</p>
              <div className="flex items-center gap-2 mt-0.5">
                <PriceTag value={total + fees} className="text-[16px] font-bold" />
                <span className="text-[11px] text-[#FAFAF7]/50">incl. fees</span>
              </div>
            </div>
    ...
    <div className="min-h-screen bg-[#FAFAF7]">
      ...
      <header className="bg-white border-b border-[#E8E9EB] sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center gap-6">
          <a href="#" className="text-[16px] font-bold tracking-tight text-[#111111] shrink-0">
            resale<span className="font-normal text-[#292B2F]">.co</span>
          </a>
          <div className="flex-1 max-w-md">
            <div className="relative">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-[#292B2F]" width="14" height="14" viewBox="0 0 14 14" fill="none">
          ...
          <nav className="hidden md:flex items-center gap-5 ml-auto">
            <a href="#" className="text-[13px] font-medium text-[#292B2F] hover:text-[#111111] transition-colors">Sell Tickets</a>
            <a href="#" className="text-[13px] font-medium text-[#292B2F] hover:text-[#111111] transition-colors">My Account</a>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="mb-8 border border-[#E8E9EB] rounded-lg overflow-hidden bg-white">
          <div className="grid md:grid-cols-2">
            <div className="p-8 flex flex-col justify-center">
              <p className="text-[11px] font-medium tracking-widest uppercase text-[#292B2F] mb-3">New York City · September 2026</p>
              <h1 className="text-3xl font-bold text-[#111111] leading-tight mb-3">
                The best seats.<br />
                No surprises.
              </h1>
              <p className="text-[14px] text-[#292B2F] leading-relaxed mb-6">
                Guaranteed authentic tickets to every concert, game, and show — with transparent pricing and no hidden fees.
              </p>
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1.5 text-[12px] text-[#292B2F]">
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 1L8.5 5H13L9.5 7.5L11 12L7 9.5L3 12L4.5 7.5L1 5H5.5L7 1Z" fill="#111111"/></svg>
                  <span>Verified sellers</span>
                </div>
                <span className="w-px h-3 bg-[#E8E9EB]" />
                <div className="flex items-center gap-1.5 text-[12px] text-[#292B2F]">
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><rect x="1" y="3" width="12" height="9" rx="1.5" stroke="#111111" strokeWidth="1.3"/><path d="M1 6H13" stroke="#111111" strokeWidth="1.3"/></svg>
                  <span>Instant e-delivery</span>
                </div>
                <span className="w-px h-3 bg-[#E8E9EB]" />
                <div className="flex items-center gap-1.5 text-[12px] text-[#292B2F]">
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><circle cx="7" cy="7" r="5.5" stroke="#111111" strokeWidth="1.3"/><path d="M4.5 7L6.5 9L9.5 5" stroke="#111111" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  <span>Money-back guarantee</span>
                </div>
              </div>
            </div>
            <div className="relative bg-[#E8E9EB] hidden md:block" style={{ minHeight: '240px' }}>
              <img
        ...
        <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-1">
          ...
          <div className="ml-auto shrink-0 flex items-center gap-2">
            <span className="text-[12px] text-[#292B2F] font-mono-data">{filtered.length} events</span>
          </div>
        </div>
        ...
        {filtered.length > 0 ? (
          ...
          <div className="text-center py-20 border border-[#E8E9EB] rounded-lg bg-white">
            <p className="text-[14px] font-medium text-[#111111] mb-1">No events found</p>
            <p className="text-[13px] text-[#292B2F]">Try a different category or search term.</p>
          </div>
        )}

        {/* How it works */}
        <div className="mt-12 mb-2">
          <h2 className="text-[13px] font-semibold uppercase tracking-wide text-[#111111] mb-4">How It Works</h2>
          <div className="grid md:grid-cols-3 gap-4">
            {[
              { step: '01', title: 'Find Your Event', desc: 'Browse thousands of events by date, location, and category. Filter by price range and seat preference.' },
              { step: '02', title: 'Choose Your Seats', desc: 'Pick from verified listings with transparent pricing. All fees shown upfront — no surprises at checkout.' },
              { step: '03', title: 'Get Your Tickets', desc: 'Receive your tickets instantly via email. Every ticket is guaranteed authentic or your money back.' },
            ].map(({ step, title, desc }) => (
              <div key={step} className="bg-white border border-[#E8E9EB] rounded-lg p-5">
                <p className="font-mono-data text-[11px] font-medium text-[#292B2F] mb-3">{step}</p>
                <h3 className="text-[14px] font-semibold text-[#111111] mb-2">{title}</h3>
                <p className="text-[13px] text-[#292B2F] leading-relaxed">{desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <footer className="border-t border-[#E8E9EB] mt-12 bg-white">
        <div className="max-w-6xl mx-auto px-6 py-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <p className="text-[14px] font-bold text-[#111111]">resale<span className="font-normal text-[#292B2F]">.co</span></p>
            <p className="text-[12px] text-[#292B2F] mt-1">© 2026 Resale Co. All rights reserved.</p>
          </div>
          <div className="flex items-center gap-6">
            {['Privacy Policy', 'Terms of Service', 'Seller FAQ', 'Contact'].map(link => (
              <a key={link} href="#" className="text-[12px] text-[#292B2F] hover:text-[#111111] transition-colors">
                {link}
              </a>
            ))}
          </div>
